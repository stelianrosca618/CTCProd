...
function OpenTillPass_Popup(data){
  data.content = typeof data.content == "undefined" ? "" : data.content;

  var $popupHTML = $("<div/>", {
    id: "system-popup",
    class: "system-popup proposal-popup",
  }).appendTo("body");

  var overlayHTML = "";
  overlayHTML += '<div class="popup-modal fadeIn animated tw-p-6">';
  ...
  overlayHTML += "</div>";
  overlayHTML += '<div class="popup-action">'
  overlayHTML += '<button type="button" id="popup_ok" class="btn btn-primary tw-mr-2">Yes</button>';
  overlayHTML += '<button type="button" id="popup_cancel" class="btn btn-default">No</button>';
  overlayHTML += '</div>'
  overlayHTML += "</div>";

  $popupHTML.html(overlayHTML).removeClass("hide");
  $("body").addClass("system-popup");
  $popupHTML.find("#popup_ok").on('click', function(){
    var that = this;
    setTimeout(function () {
    ...
    requestGet(
      "proposals/get_" + data.selector + "_convert_data/" + proposal_id
    ).done(function (response) {
      ...
      $("#convert_to_" + data.selector).modal({
        show: true,
        backdrop: "static",
      });
      reorder_items();
  ...
  $popupHTML.find("#popup_cancel").on("click", function () {
    var that = this;
    setTimeout(function () {
      $("body").removeClass("system-popup");
      $popupHTML.fadeOut(400, function () {
...
function proposal_convert_template(invoker) {
  ...
  console.log("Invoice Get", template, invoker);
  $.get(admin_url + "proposals/getProposalOpenTill/"+proposal_id).done(function(data){
    response = JSON.parse(data);
    console.log(response, data[0], response.open_till);
    var openTillDate = new Date(response.open_till);
    var nowDate = new Date();
    if(openTillDate.getTime() < nowDate.getTime()){
      alert_float("danger", 'Date Is passed');
      var $p = OpenTillPass_Popup({
        message: "The Validity day already pass!",
        content: "Do you want to create the invoice with passed date?",
        selector: html_helper_selector
      });
    }else{
      requestGet(
...
function calculate_total() {
  ...
  $.each(rows, function () {
    ...
    console.log('check Input', $(this).find("td.rate input#rate"));
    if($(this).find("td.rate input#rate").hasClass('hide')){
      _amount = 0.0;  
    }else{
      _amount = accounting.toFixed(
        $(this).find("td.rate input#rate").val() * quantity,
    ...
    console.log('amount testing', $(this).find('.incoterm-input'));
    if($(this).find('.incoterm-input').length > 0){
      ...
      $(this).find('.incoterm-input').each((id, ele) => {
        ...
        if(!ele.classList.contains("hide")){
          ...
          _interComAmount += parseFloat(inCoTermval);
        }
      })
      _amount += _interComAmount
    }
