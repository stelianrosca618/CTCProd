...
class Invoice_items extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['item_subGroups']= $this->db->get(db_prefix().'items_subgroup')->result_array();
    ...
    public function get_item_by_id($id)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->db->where('itemGroup_id', $item->group_id);
            $this->db->from(db_prefix().'stocks');
            $item->totalQuantity = $this->db->get()->result_array()[0];
