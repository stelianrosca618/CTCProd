...
class Proposals extends AdminController
{
    ...
    public function getProposalOpenTill($id){
        $this->db->select('open_till');
        $this->db->where('id', $id);
        $opentTillData = $this->db->get(db_prefix().'proposals')->result_array();

        echo json_encode($opentTillData[0]);
    }
    ...
    public function get_proposal_data_ajax($id, $to_return = false)
    {
        ...
        if ($data['proposal']->invoice_id > 0) {
            $data['proposal']->show_invoice_qty = true;
        } else {
            $data['proposal']->show_invoice_qty = false;
        }
    ...
    public function convert_to_invoice($id)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($invoice_id) {
                ...
                $this->db->where('rel_id', $id)->where('rel_type', 'proposal');
                ...
                foreach($postedData['newitems'] as $proItem){
                    ...
                    $prodRatio = $isItem[0]['production_ratio']? $isItem[0]['production_ratio'] : 100;
    ...
    public function repeat($id)
    {
        ...
        $new_id = $this->proposals_model->repeat($id);
