...
class Staff extends AdminController
{
    ...
    public function updateMailingForProposal(){
        $data = $this->input->post();
        if($data){
            
            $this->db->where('staffid', $data['id'])->update(db_prefix().'staff', [
                'for_proposal'=>$data['forProposal']
            ]);
            echo true;
        }
        echo false;
    ...
    public function updateMailingForContract(){
        $data = $this->input->post();
        if($data){
            $this->db->where('staffid', $data['id'])->update(db_prefix().'staff', [
                'for_contract'=>$data['forContract']
            ]);
            echo true;
        }
        echo false;
