...
<div id="wrapper">
    ...
    echo ' hide';
                              ...
                              ?>
                                ...
                                <div>
                                    ...
                                    $proposalChecked = '';
                                    $contractChecked = '';
                                    if(isset($member)&& $member->for_proposal){
                                        $proposalChecked = 'checked';
                                    }
                                    if(isset($member)&& $member->for_contract){
                                        $contractChecked = 'checked';
                                    }
                                    ?>
                                    <input type="checkbox" id="forProposal" name="for_proposal" <?php echo $proposalChecked;?>>
                                    <label for="forProposal">For Proposal</label>
                                    <input type="checkbox" id="forContract" name="for_contract"  <?php echo $contractChecked;?>>
                                    <label for="forContract">For Contract</label>
                                </div>
    ...
    <script>
    $(function() {
        $('#forProposal').on('change', function(e) {
            console.log(e);
            var payload = {
                id: $('input[name="memberid"]').val(),
                forProposal: e.target.checked? 1 : 0
            }
            $.post(admin_url + "staff/updateMailingForProposal", payload);
        })
        $('#forContract').on('change', function(e) {
            console.log(e);
            var payload = {
                id: $('input[name="memberid"]').val(),
                forContract: e.target.checked? 1 : 0
            }
            $.post(admin_url + "staff/updateMailingForContract", payload);
        })
