...
<div class="<?php if (!isset($invoice) || (isset($invoice) && count($invoices_to_merge) == 0 && (isset($invoice) && !isset($invoice_from_project) && count($expenses_to_bill) == 0 || $invoice->status == Invoices_model::STATUS_CANCELLED))) {
    echo ' hide';
} ?>" id="invoice_top_info" style="display: none;">
    ...
    <div class="panel-body">
        ...
        <div class="row">
            ...
            if (!isset($invoice_from_project)) { ?>
                ...
                <?php
               $next_invoice_number = isset($proposal)? $proposal->id : get_option('next_invoice_number');
    ...
    <div class="panel-body">
        ...
        <div class="table-responsive s_table" style="overflow: auto;">
            <table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop">
                  ...
                  foreach ($custom_fields as $cf) {
                     ...
                     ?>
                        ...
                        <th width="15%" align="right">
                            ...
                            // VK Mod: Add
                            if (!empty($proposal) && !empty($proposal->incoterms) && ((isset($proposal->incoterms['fob_port']) && $proposal->incoterms['fob_port']) || (isset($proposal->incoterms['cfr_port']) && $proposal->incoterms['cfr_port'])) && isset($proposal->incoterms['container_type']) && $proposal->incoterms['container_type'] && isset($proposal->incoterms['ports']) && $proposal->incoterms['ports']) {
                                
                                //$htmlOption = '<li><a href="#" class="rate-list" data-id="rate" data-name="' . _l('invoice_table_rate_heading') . '">' . _l('invoice_table_rate_heading') . '</a></li>';
                                $htmlOption = '<option class="rate-list" value="rate" selected>'. _l('invoice_table_rate_heading') .'</option>';
                                foreach ($proposal->incoterms['fob_port'] as $fob_port) {
                                    $filterId = $fob_port;
                                    $port = array_filter($proposal->incoterms['ports'], function ($var) use ($filterId) {
                        ...
                        if(isset($invoice->incoterms)){
                            foreach ($invoice->incoterms['fob_port'] as $fob_port) {
                             ...
                             //       $htmlOption .= '<option class="rate-list" value="fob-'.$fob_port.'-'.strtolower(str_replace(' ', '-', $container_type)).'">FOB ('.$port_name.') '.$prevContainer.'</option>';
                                    ?>
                                    <th width="20%" align="right"><?php echo ''.$prevContainer.''; ?></th>
                                    <?php
                                }
                            }
                ...
                <tbody>
                    <tr class="main <?php echo (!empty($proposal) && !empty($proposal->incoterms)) ? 'hide' : ''; ?>">
                        <td></td>
                        <td>
                    ...
                    <?php if (isset($invoice) || isset($add_items)) {
                            ...
                            foreach ($add_items as $item) {
                                ...
                                // order input
                                $table_row .='<input type="hidden" class="referneceItem" name="' . $items_indicator . '[' . $i . '][reference_item_id]">';
                                ...
                                /// BOF VK, Handle proposal incoterms QTY
                                ...
                                $table_row .= '</td>';
                                $table_row .= '<td class="rate rate-port" data-key="'.$i.'">
                                <input type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][rate]" value="' . $item['rate'] . '" id="rate" class="form-control"><label for="rate">Rate</label>';
