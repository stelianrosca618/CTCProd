...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    // $isItem = $CI->db->get(db_prefix().'items')->result_array();
    // $prodRatio = $isItem['production_ratio']? $isItem['production_ratio'] : 100;
    // $CI->db->insert(db_prefix().'stocks', [
    //     'quantity'    => $item['qty'] * ($prodRatio / 100) * -1,
    //     'date' => date("Y-m-d"),
    //     'status' => 'invoiced',
    //     'Notes' => get_option('proposal_number_prefix') . str_pad($proposalData[0]['id'], get_option('number_padding_prefixes'), '0', STR_PAD_LEFT).' CTC',
