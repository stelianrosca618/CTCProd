...
class Invoice_items_model extends App_Model
{
    ...
    public function copy($_data)
    {
        ...
        $data = [
            ...
            // VK Mod: Add
            'fcl_20_container' => $_data['fcl_20_container'],
            'fcl_40_container' => $_data['fcl_40_container'],
            'air_container'    => $_data['air_container'],
    ...
    public function get($id = '')
    {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid,rate,
            ...
            t2.taxrate as taxrate_2,t2.id as taxid_2,t2.name as taxname_2, prod_link,
            description,long_description,group_id, subgroup_id, production_ratio,'.db_prefix().'items_groups.limitQty,'.db_prefix().'items_groups.isAlert,' . db_prefix() . 'items_groups.name as group_name,
            '.db_prefix().'items_subgroup.name as subgroup_name, 
            unit,fcl_20_container,fcl_40_container,air_container');
        // EOF VK, Add three fields
        ...
        $this->db->join(db_prefix() . 'items_subgroup', db_prefix() . 'items_subgroup.id='.db_prefix() . 'items.subgroup_id', 'left');
    ...
    /*public function get_grouped_itemable($groupID){
        $this->db->where('group_id', $groupID);
        $this->db->join(db_prefix().'itemable', db_prefix().'itemable.item_order = ' . db_prefix().'items.id', 'left');
        $this->db->join(db_prefix().'proposals', db_prefix().'proposals.id = '.db_prefix().'itemable.rel_id', 'left');
        $groupdData = $this->db->get(db_prefix() . 'items')->result_array();
