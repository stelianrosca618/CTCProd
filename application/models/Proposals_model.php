...
class Proposals_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            // VK Mod: Add
            handle_proposal_incoterms_save($insert_id, $hook['incoterms']);
            ...
            // BOF VK, Handle item incoterms rate
            // VK Mod: Add
            delete_itemable_incoterms_data($insert_id);
            // EOF VK, Handle item incoterms rate
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'proposal')) {
                    ...
                    // VK Mod: Add
                    if ($hook['incoterms']) {
                        handle_itemable_incoterms_save($insert_id, $itemid, $hook['incoterms'], $item['incoterms']);
                    }
                    // EOF VK, Handle item incoterms rate
    ...
    public function update($data, $id)
    {
        ...
        // BOF VK, Handle proposal incoterms data
        // VK Mod: Add
        handle_proposal_incoterms_save($id, $hook['incoterms']);
        // EOF VK, Handle proposal incoterms data
        ...
        // BOF VK, Handle item incoterms rate
        // VK Mod: Add
        delete_itemable_incoterms_data($id);
        // EOF VK, Handle item incoterms rate
        ...
        foreach ($items as $key => $item) {
            ...
            // VK Mod: Add
            if ($hook['incoterms']) {
                handle_itemable_incoterms_save($id, $item['itemid'], $hook['incoterms'], $item['incoterms']);
            }
            // EOF VK, Handle item incoterms rate
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'proposal')) {
                ...
                // VK Mod: Add
                if ($hook['incoterms']) {
                    handle_itemable_incoterms_save($id, $new_item_added, $hook['incoterms'], $item['incoterms']);
                }
                // EOF VK, Handle item incoterms rate
    ...
    public function copy($id)
    {
        ...
        // BOF VK, Handle proposal incoterms data
        // VK Mod: Add
        if ($proposal->incoterms) {
            $insert_data['fob_port'] = $proposal->incoterms['fob_port'];
            $insert_data['cfr_port'] = $proposal->incoterms['cfr_port'];
            $insert_data['container_type'] = $proposal->incoterms['container_type'];
        }
        // EOF VK, Handle proposal incoterms data
        ...
        foreach ($proposal->items as $item) {
            ...
            // BOF VK, Handle proposal incoterms data
            // VK Mod: Add
            if ($proposal->items_incoterms) {
                $filterId = $item['id'];
                $item_incoterms = array_filter($proposal->items_incoterms, function ($var) use ($filterId) {
                    return ($var['item_id'] == $filterId);
                });

                $item_incoterms = array_values($item_incoterms);

                if (is_array($item_incoterms) && count($item_incoterms) > 0) {
                    foreach ($item_incoterms as $incoterm) {
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_fcl_20'] = $incoterm['rate_fob_fcl_20'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_fcl_40'] = $incoterm['rate_fob_fcl_40'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_air'] = $incoterm['rate_fob_air'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_fcl_20'] = $incoterm['rate_cfr_fcl_20'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_fcl_40'] = $incoterm['rate_cfr_fcl_40'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_air'] = $incoterm['rate_cfr_air'];
                    }
                }
            }
            // EOF VK, Handle proposal incoterms data
    ...
    public function repeat($id)
    {
        ...
        $insert_data['subject']     = 'Proposal '.time();
        $insert_data['addedfrom']   = get_staff_user_id();
        $insert_data['datecreated'] = date('Y-m-d H:i:s');
        $insert_data['date']        = _d(date('Y-m-d'));
        $insert_data['status']      = 6;
        ...
        // BOF VK, Handle proposal incoterms data
        // VK Mod: Add
        // if ($proposal->incoterms && isset($proposal->incoterms['exchange_rate'])) {
        ...
        foreach ($proposal->items as $item) {
            ...
            // VK Mod: Add
            $db_item = array();
            if ($item['reference_item_id'] > 0) {
                $this->db->select(db_prefix() . 'items.id as itemid,rate,
                    description,long_description,group_id,unit,fcl_20_container,fcl_40_container,air_container');
                $this->db->from(db_prefix() . 'items');            
                $this->db->where(db_prefix() . 'items.id', $item['reference_item_id']);

                $db_item =  $this->db->get()->row_array();
            }
            // EOF VK, Handle proposal incoterms data

            $insert_data['newitems'][$key]['reference_item_id']= $item['reference_item_id'];
            $insert_data['newitems'][$key]['description']      = $item['description'];
            $insert_data['newitems'][$key]['long_description'] = clear_textarea_breaks($item['long_description']);
            $insert_data['newitems'][$key]['qty']              = $item['qty'];
            $insert_data['newitems'][$key]['unit']             = $item['unit'];
            ...
            $insert_data['newitems'][$key]['rate']  = ($db_item) ? $db_item['rate'] : $item['rate'];
            ...
            if ($proposal->items_incoterms) {
                ...
                if (is_array($item_incoterms) && count($item_incoterms) > 0) {
                    foreach ($item_incoterms as $incoterm) {
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_fcl_20'] = ($db_item) ? $db_item['rate'] : $item['rate'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_fcl_40'] = ($db_item) ? $db_item['rate'] : $item['rate'];
                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_fob_air'] = ($db_item) ? $db_item['rate'] : $item['rate'];

                        $portId = $incoterm['freight_id'];
                        $port_incoterms = array_filter($proposal->incoterms['ports'], function ($var) use ($portId) {
                            return ($var['id'] == $portId);
                        });

                        if ($port_incoterms) {
                            $port_incoterms = array_values($port_incoterms);
                            $port_incoterm = $port_incoterms[0];
                        } else {
                            $port_incoterm = array();
                        }

                        $cfr_fcl_20 = ($db_item) ? $db_item['rate'] : $item['rate'];
                        if ($port_incoterm && $db_item && $port_incoterm['fcl_20'] > 0 && $db_item['fcl_20_container'] > 0) {
                            $cfr_fcl_20 = round($cfr_fcl_20 + ($port_incoterm['fcl_20']/$db_item['fcl_20_container']));
                        }

                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_fcl_20'] = $cfr_fcl_20;

                        $cfr_fcl_40 = ($db_item) ? $db_item['rate'] : $item['rate'];
                        if ($port_incoterm && $db_item && $port_incoterm['fcl_40'] > 0 && $db_item['fcl_40_container'] > 0) {
                            $cfr_fcl_40 = round($cfr_fcl_40 + ($port_incoterm['fcl_40']/$db_item['fcl_40_container']));
                        }

                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_fcl_40'] = $cfr_fcl_40;

                        $cfr_air = ($db_item) ? $db_item['rate'] : $item['rate'];
                        if ($port_incoterm && $db_item && $port_incoterm['air'] > 0 && $db_item['air_container'] > 0) {
                            $cfr_air = round($cfr_air + ($port_incoterm['air']/$db_item['air_container']));
                        }

                        $insert_data['newitems'][$key]['incoterms'][$incoterm['freight_id']]['rate_cfr_air'] = $cfr_air;
                    }
                }
            }
            // EOF VK, Handle proposal incoterms data
    ...
    public function get_relation_data_values($rel_id, $rel_type)
    {
        ...
        if ($rel_type == 'customer') {
            ...
            $this->db->join(db_prefix().'customer_groups', db_prefix().'customer_groups.customer_id='.$rel_id, 'left');
            $this->db->join(db_prefix().'customers_groups', db_prefix().'customers_groups.id='.db_prefix().'customer_groups.groupid', 'left');
